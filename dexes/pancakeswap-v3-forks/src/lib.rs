// PancakeSwap V3 Substreams Implementation
//
// This implementation is specifically for PancakeSwap V3 pools across different blockchains:
// - PancakeSwap V3 (BSC, Ethereum, Arbitrum, zkSync Era, Polygon zkEVM)
//
// IMPORTANT: Event Signature Difference
// --------------------------------------
// PancakeSwap V3 uses a DIFFERENT Swap event signature than Uniswap V3:
// - Adds protocolFeesToken0 and protocolFeesToken1 parameters
// - Event signature: 0x19b47279256b2a23a1665c810c8d55a1758940ee09377d4f8d26497a3577dc83
// - Data size: 224 bytes (vs 160 for Uniswap V3)
//
...
// PancakeSwap V3 Swap(address,address,int256,int256,uint160,uint128,int24,uint128,uint128) - with indexed sender/recipient
// Note: PancakeSwap V3 adds protocolFeesToken0 and protocolFeesToken1 parameters compared to Uniswap V3
...
fn process_swap_event(log: &LogView, pool_aggregations: &mut HashMap<Vec<u8>, SwapAggregation>) {
    // PancakeSwap V3 Swap event structure:
    ...
    // Data layout: 32 + 32 + 32 + 32 + 32 + 32 + 32 = 224 bytes minimum
    if log.data().len() < 224 || log.topics().len() < 3 {
        return;
    }
